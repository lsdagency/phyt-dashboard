"use client";

import { useState } from "react";
import type { DashboardData } from "@/lib/integrations/types";
import { money, num, pct } from "@/lib/format";

type SortKey = "spend" | "installs" | "subscriptions" | "cpa" | "costPerSub";

export default function KeywordTable({
  data,
  bidOverride,
}: {
  data: DashboardData;
  // Optional per-keyword proposed bids (e.g. from a Claude regeneration) that
  // override the baseline heuristic bids baked into the rows.
  bidOverride?: Record<string, { proposedBid: number; action: "increase" | "decrease" }>;
}) {
  const c = data.currency;
  const [sort, setSort] = useState<SortKey>("spend");

  const rows = [...data.asa.keywords].sort((a, b) => b[sort] - a[sort]);

  return (
    <div>
      <div className="mb-2 flex items-center gap-2 text-xs text-phyt-ink/60">
        Sort by:
        {(
          [
            ["spend", "Spend"],
            ["installs", "Installs"],
            ["subscriptions", "Subs"],
            ["cpa", "CPI"],
            ["costPerSub", "Cost/Sub"],
          ] as [SortKey, string][]
        ).map(([k, label]) => (
          <button
            key={k}
            onClick={() => setSort(k)}
            className={`rounded-full px-2.5 py-1 ${
              sort === k ? "bg-phyt-yellow font-medium" : "bg-phyt-ink/5"
            }`}
          >
            {label}
          </button>
        ))}
      </div>

      <div className="overflow-x-auto rounded-2xl border border-phyt-ink/10">
        <table className="w-full min-w-[1100px] text-sm">
          <thead>
            <tr className="border-b border-phyt-ink/10 bg-phyt-ink/[0.02] text-left text-[11px] uppercase tracking-wide text-phyt-ink/55">
              <th className="px-3 py-3">Keyword</th>
              <Th>Bid</Th>
              <Th>CPT</Th>
              <Th>Spent</Th>
              <Th>Impr.</Th>
              <Th>Taps</Th>
              <Th>Installs</Th>
              <Th>CPI</Th>
              <Th>Trials</Th>
              <Th>Cost/Trial</Th>
              <Th>Subs</Th>
              <Th>Cost/Sub</Th>
              <Th>Proposed bid</Th>
            </tr>
          </thead>
          <tbody>
            {rows.map((x) => (
              <tr key={x.keywordId} className="border-b border-phyt-ink/5 hover:bg-phyt-ink/[0.015]">
                <td className="px-3 py-2.5">
                  <div className="font-medium">{x.keyword}</div>
                  <div className="text-[11px] text-phyt-ink/50">
                    {x.matchType} · {x.campaignName}
                  </div>
                </td>
                <Td>{money(x.bid, c)}</Td>
                <Td>{money(x.cpt, c)}</Td>
                <Td>{money(x.spend, c)}</Td>
                <Td>{num(x.impressions)}</Td>
                <Td>{num(x.taps)}</Td>
                <Td>{num(x.installs)}</Td>
                <Td>{money(x.cpa, c)}</Td>
                <Td>{num(x.trials)}</Td>
                <Td>{money(x.costPerTrial, c)}</Td>
                <Td>{num(x.subscriptions)}</Td>
                <Td>{money(x.costPerSub, c)}</Td>
                <td className="px-3 py-2.5 text-right">
                  {(() => {
                    const ov = bidOverride?.[x.keywordId];
                    const proposed = ov ? ov.proposedBid : x.proposedBid;
                    const action = ov ? ov.action : x.bidAction;
                    return <ProposedBid bid={x.bid} proposed={proposed} action={action} currency={c} />;
                  })()}
                </td>
              </tr>
            ))}
          </tbody>
        </table>
      </div>
      <p className="mt-2 text-xs text-phyt-ink/50">
        Proposed bids are generated by Claude&apos;s daily optimisation pass (Step 5).
      </p>
    </div>
  );
}

function ProposedBid({
  proposed,
  action,
  currency,
}: {
  bid: number;
  proposed: number | null;
  action: "increase" | "decrease" | "hold" | null;
  currency: string;
}) {
  if (proposed == null) return <span className="text-phyt-ink/30">—</span>;
  const tint =
    action === "increase"
      ? "bg-phyt-green-soft"
      : action === "decrease"
        ? "bg-phyt-pink-soft"
        : "bg-phyt-ink/5";
  const arrow = action === "increase" ? "↑" : action === "decrease" ? "↓" : "→";
  return (
    <span className={`rounded-full px-2.5 py-0.5 text-xs tabular-nums ${tint}`}>
      {arrow} {money(proposed, currency)}
    </span>
  );
}

function Th({ children }: { children: React.ReactNode }) {
  return <th className="px-3 py-3 text-right">{children}</th>;
}
function Td({ children }: { children: React.ReactNode }) {
  return <td className="px-3 py-2.5 text-right tabular-nums">{children}</td>;
}
